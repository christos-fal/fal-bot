...
class RateLimiter:
    ...
    def _clean_old_timestamps(self, user_id: int):
        ...
        if user_id in self.daily_usage:
            self.daily_usage[user_id] = [
                ts for ts in self.daily_usage[user_id] 
                if ts > cutoff
            ]
    ...
    def can_generate(self, user_id: int) -> tuple[bool, str]:
        ...
        if user_id in self.active_users:
            return False, "You already have a generation in progress. Please wait for it to complete."
        ...
        if used >= self.DAILY_LIMIT:
            ...
            return False, f"Daily limit reached ({self.DAILY_LIMIT} generations/day). Resets in {hours}h {minutes}m."
